    ...
    if ( $index_type eq 'delta' ) {
        ...
        if ( my $delta = $index_hash->{delta} ) {
            if ( exists $index_hash->{date_columns}->{$delta} ) {
                $args{sort} = $delta;
                $args{start_val} = epoch2ts( undef, time - ( 36 * 60 * 60 ) )
                    ;          # 36 hours prior to current
            }
        }
    }
    my %terms;
        ...
        if ( $index_hash->{mva} ) {
            ...
            foreach my $mva ( keys %{ $index_hash->{mva} } ) {
                ...
                if ( my $mva_query = $cur_mva->{query} ) {
                    ...
                    eval("require $mva_class;") if ($mva_class);  ## no critic
                    next if ($@);
                    ...
                    return 0
                        if !$sth
                    ;    # ignore this operation if _meta column doesn't exist
                    $sth->execute or next;
                    ...
                    if ( my $id_columns = $cur_mva->{by} ) {
                        ...
                        %terms = ( "${mva}_"
                                . $index_hash->{id_column} => $obj->id );
                    }

                    my $mva_source = $cur_mva->{with};
                    next if ( !$mva_source || $mva_source eq '' );

                    eval("require $mva_source");    ## no critic
...
"SELECT count(*) from mt_$what_ds WHERE ${what_ds}_$with_column = ${source}_"
        ...
        if ( $index_hash->{mva} ) {
            foreach my $mva ( keys %{ $index_hash->{mva} } ) {
                ...
                if ( ref($cur_mva) ) {
                    if ( !$cur_mva->{query} ) {
                        eval( "use " . $cur_mva->{with} );    ## no critic
        ...
        my %fields;
        @fields{ ( $index_hash->{id_column}, @{ $index_hash->{columns} } ) }
            = ( $index_hash->{id_column}, @{ $index_hash->{columns} } );
        foreach ( keys %fields ) {
...
# delete $fields{$_} if ($index_hash->{string_group_columns}->{$_});
        ...
        delete $fields{ $index_hash->{id_column} }
            if ( $index_hash->{id_column} );
...
sub _gen_sphinx_conf_tmpl {
    ...
    if ( $conf_type eq '' || $conf_type !~ /xml/ ) {
        ...
        $tmpl = $plugin->load_tmpl('sphinx.xpipe.conf.tmpl')
            or die $plugin->errstr
            if ( $conf_type ne '' && $conf_type =~ /xml/ );
    }
    ...
    if ( $app->can('document_root') ) {
        ...
        $params{mtinstallpath} = $app->{cfg}->installpath
            || '/MISSING/PATH/TO/MT/INSTALL/BASE';
    }
    ...
    my $iter
        = MT->model('entry')
        ->count_group_by( { class => '*', status => MT::Entry::RELEASE() },
        { group => ['blog_id'] } );
    my $entry_count;
